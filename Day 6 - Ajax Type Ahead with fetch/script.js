...
function diplaySuggestions(){
    ...
    const html = places.map(place => {
        ...
        return `
            <li>
                <span>${cityName}, ${stateName}</span><span>${numberWithCommas(place.population)}</span>
            </li>
            `;
    }).join('');
    console.log(places);
...
inputField.addEventListener('change', diplaySuggestions);
inputField.addEventListener('keyup', diplaySuggestions);